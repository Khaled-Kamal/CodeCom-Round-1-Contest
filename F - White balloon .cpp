...
void solve()
{
    ...
    sort(all(temp));
    for (int i = 0; i < n; ++i) {
        if(temp[i] != perm[i]) pos1[i] = 1;
    }
    sort(all_r(temp));

    for (int i = 0; i < n; ++i) {
        if(temp[i] != perm[i]) pos2[i] = 1;
    }

    set<int> p1, p2, both;
    for (int i = 0; i < n; ++i) {
        if(pos1[i] == 1 && pos2[i] == 1) both.insert(i);
        else if(pos1[i] == 1) p1.insert(i);
        else if(pos2[i] == 1) p2.insert(i);
    }

    int p = 1;
    while (p1.size() || p2.size() || both.size()) {

        if(p1.empty() && both.empty()) {
            return void(cout << "First\n");
        }
        if(p2.empty() && both.empty()) {
            return void(cout << "Second\n");
        }
        if(p > 3 * n)
            break;
        if(p&1) {
            if(p1.size()) p1.erase(p1.begin());
            else if(both.size()) {
                if(p2.empty() && both.size() == 1) {
                    p++;
                    continue;
                }
            ...
            if(p2.size()) p2.erase(p2.begin());
            else if(both.size()) {
                if(p1.empty() && both.size() == 1) {
                    p++;
                    continue;
                }
                both.erase(both.begin());
